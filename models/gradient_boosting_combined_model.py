...
class GradientBoostingCombinedPredictor(BasePredictor):
    """
    Двухстадийная Combined модель с Gradient Boosting.

    АРХИТЕКТУРА:
    Стадия 1 (slope_predictor): GB → (f0, slope)
    Стадия 2 (coeff_predictor): GB → (a, d)

    Полностью аналогична RandomForestCombinedPredictor, но использует
    Gradient Boosting. Обычно точнее линейных моделей, но медленнее.
    Использует нативный multi-output GB для учёта корреляций.
    """
    ...
    def train(self, test_size=0.2, n_iter_slope=50, n_iter_coeff=50, cv_splits=5):
        ...
        if self.verbose:
            print(f"\n{'=' * 80}")
            print(f"GB COMBINED MODEL: обучение на {len(X_temp)} samples, тест на {len(X_test)} samples")
            print(f"{'=' * 80}\n")
